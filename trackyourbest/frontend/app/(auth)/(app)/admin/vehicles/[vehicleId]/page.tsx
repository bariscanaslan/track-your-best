...
export default function AdminVehicleEditPage() {
  ...
  // Track the device that was loaded with this vehicle to detect changes
  const originalDeviceId = useRef<string>("");
  ...
  const fetchVehicle = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(vehiclesApi.getByIdAdmin(vehicleId, apiBase), { credentials: "include" });
      if (!res.ok) throw new Error("Failed to load vehicle.");
      const data: VehicleDetail = await res.json();
      setVehicle(data);
      originalDeviceId.current = data.deviceId ?? "";
      setForm({
        ...
        inspectionExpiry: toDateInput(data.inspectionExpiry),
      ...
      try {
        ...
        if (vehRes.status === "fulfilled" && vehRes.value.ok) {
          ...
          const ids = new Set<string>(
            Array.isArray(data)
              ? data
                  .filter((v: { deviceId?: string | null; id?: string }) => v.deviceId && v.id !== vehicleId)
                  .map((v: { deviceId: string }) => v.deviceId)
              : []
          );
  ...
  const availableDevices = useMemo(() => {
    return allDevices.filter((d) => {
      ...
      // Always include the device currently assigned to this vehicle
      if (d.id === originalDeviceId.current) return true;
  ...
  const handleSave = async () => {
    ...
    // Warn when changing to a different device
    const isChangingDevice = form.deviceId !== originalDeviceId.current && form.deviceId !== "";
    if (isChangingDevice) {
      const currentDeviceName = allDevices.find((d) => d.id === originalDeviceId.current)?.deviceName ?? originalDeviceId.current;
      const newDeviceName = allDevices.find((d) => d.id === form.deviceId)?.deviceName ?? form.deviceId;
      const confirmed = window.confirm(
        `You are changing the device from "${currentDeviceName}" to "${newDeviceName}".\n\nThe current device will be deactivated and a new cloned device will be created. This cannot be undone.\n\nProceed?`
      );
      if (!confirmed) return;
    }
    ...
    setError(null);
    try {
      const res = await fetch(vehiclesApi.update(vehicle.id, apiBase), {
        method: "PUT",
  ...
  return (
    <div className="adm-page">
      ...
      <div className="adm-edit-banner">
        <div>
          <strong>Editing Vehicle</strong>
          <div className="adm-list-sub">Update vehicle details, device and organization assignment.</div>
        </div>
        <div className="adm-list-sub">Vehicle ID: {vehicleId}</div>
      </div>

      {isLoading && <div className="adm-note">Loading vehicle...</div>}
                ...
                <select className="adm-select" value={form.deviceId} onChange={handleChange("deviceId")}>
                  ...
                  {availableDevices.map((d) => (
                    <option key={d.id} value={d.id}>
                      {d.deviceName}{d.id === originalDeviceId.current ? " (current)" : ""}
                    </option>
                  ))}
                </select>
                {form.deviceId !== originalDeviceId.current && form.deviceId !== "" && (
                  <div className="adm-note adm-warning-danger" style={{ marginTop: 6, padding: "6px 10px", fontSize: 12 }}>
                    Changing the device will deactivate the current one and create a new clone.
                  </div>
                )}
