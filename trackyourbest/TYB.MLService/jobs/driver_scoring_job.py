...
class DriverScoringJob:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            logger.info(f"📊 {len(unscored_trips)} trip scoring için hazır")
            ...
            for trip in unscored_trips:
                try:
                    ...
                    db.rollback()
                    logger.error(f"❌ Trip {trip.id} işlenirken hata: {exc}", exc_info=True)
    ...
    def _score_trip(self, db: Session, trip: Trip) -> None:
        ...
        if not trip.driver_id:
            logger.warning(f"⚠️ Trip {trip_id}: driver_id NULL — atlandı")
            return
        ...
        if len(gps_rows) < 10:
            logger.warning(
                f"⚠️ Trip {trip_id}: GPS verisi yetersiz "
                f"({len(gps_rows)} points, minimum 10 gerekli) — atlandı"
            )
            return
        ...
        if not features:
            logger.warning(f"⚠️ Trip {trip_id}: feature çıkarılamadı — atlandı")
            return
