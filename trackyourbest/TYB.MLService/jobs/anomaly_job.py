...
class AnomalyDetectionJob:
    ...
    # ------------------------------------------------------------------
    def run(self) -> None:
        logger.info("🔍 Anomaly Detection Job başladı...")
        db = get_session()
        try:
            ...
            for trip in trips:
                try:
                    ...
                    logger.error(f"❌ Trip {trip.id} analizi hatası: {exc}", exc_info=True)
                    db.rollback()

            logger.info("✅ Anomaly Detection Job tamamlandı")

        except Exception as exc:
            logger.error(f"❌ Anomaly Detection Job hatası: {exc}", exc_info=True)
        finally:
            db.close()
    ...
    def _analyze_trip(self, db: Session, trip: Trip) -> None:
        ...
        if len(rows) < 2:
            logger.warning(
                f"⚠️ Trip {trip.id}: GPS verisi yetersiz ({len(rows)} points) — atlandı"
            )
            return
        ...
        if not features:
            logger.warning(f"⚠️ Trip {trip.id}: feature çıkarılamadı — atlandı")
            return
        ...
        if device_id_raw is None:
            logger.warning(
                f"⚠️ Trip {trip.id}: device_id bulunamadı — anomaly kaydı atlandı"
            )
            return
