...
export default function DriverMapFooter({
  ...
  const [etaBaseTime, setEtaBaseTime] = useState<number | null>(null);

  useEffect(() => {
    const timeout = window.setTimeout(() => setEtaBaseTime(Date.now()), 0);
  ...
  const arrivalTime =
    isOngoing && eta?.predictedArrivalTime
      ...
      : isOngoing && eta?.etaMinutes != null && etaBaseTime != null
        ? formatHHmm(new Date(etaBaseTime + eta.etaMinutes * 60_000).toISOString())
        : null;
