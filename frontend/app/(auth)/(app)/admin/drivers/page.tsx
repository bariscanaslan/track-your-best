...
export default function AdminDriversPage() {
  ...
  const filteredRows = useMemo(() => {
    ...
    return sorted.filter((d) => {
      ...
      return [d.fullName, d.email, d.vehicleName, d.licenseNumber].filter(Boolean).join(" ").toLowerCase().includes(q);
    });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(driversApi.remove(id, apiBase), { method: "DELETE", credentials: "include" });
    ...
    <div className="adm-page">
      <div className="adm-header">
        ...
        <div className="adm-actions">
          ...
          <Link className="adm-button adm-button-primary" href="/admin/drivers/new">Add Driver</Link>
        </div>
      </div>
          ...
          <div key={driver.id} className="adm-list-row">
            <div className="adm-list-top">
              <div className="adm-list-identity">
                <img src={driver.avatarUrl || "/tyb-logo.png"} alt={driver.fullName || "Driver"} className="adm-avatar" />
                <div>
                  <div className="adm-list-title-row">
                    <div className="adm-list-title">{driver.fullName || driver.licenseNumber || "-"}</div>
                    <span className={`adm-status${driver.isActive === false ? " is-inactive" : ""}`}>
                      {driver.isActive === false ? "inactive" : "active"}
                    </span>
                    {driver.licenseType && <span className="adm-list-pill">{driver.licenseType}</span>}
                  </div>
                  <div className="adm-list-sub">User ID: {driver.userId ?? "-"}</div>
                </div>
              </div>
              <div className="adm-list-actions">
                <Link className="adm-link" href={`/admin/drivers/${driver.id}`}>Edit</Link>
                <button className="adm-link adm-link-danger" type="button" onClick={() => handleDelete(driver.id)}>Delete</button>
              </div>
            </div>
            <div className="adm-list-details">
              <div className="adm-list-card">
                ...
                <div className="adm-list-card-sub">{driver.vehicleId ? driver.vehicleId.slice(0, 8) + "..." : "-"}</div>
              </div>
              <div className="adm-list-card">
